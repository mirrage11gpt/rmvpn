...
Узел одновременно поднимает Hysteria 2 на UDP 443 и CDN‑совместимый VLESS
WebSocket на TCP 443. Для VLESS доменная запись локации должна быть proxied через
поддерживающий WebSocket CDN. Клиентская подписка содержит один профиль.
